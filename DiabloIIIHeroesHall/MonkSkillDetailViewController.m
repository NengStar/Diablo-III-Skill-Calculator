...
@implementation MonkSkillDetailViewController
@synthesize initiative,passive;
@synthesize primary,secondary,defensive,techniques,focus,mantras;
...
- (void)addSkillViewToScroll
{
    ...
    techniques.frame = CGRectMake(bounds.size.width*techniques.tag, 0, bounds.size.width, bounds.size.height);
    focus.frame = CGRectMake(bounds.size.width*focus.tag, 0, bounds.size.width, bounds.size.height);
    mantras.frame = CGRectMake(bounds.size.width*mantras.tag, 0, bounds.size.width, bounds.size.height);
    
    [skillScroll addSubview:primary];
    [skillScroll addSubview:secondary];
    [skillScroll addSubview:defensive];
    [skillScroll addSubview:techniques];
    [skillScroll addSubview:focus];
    [skillScroll addSubview:mantras];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    AppDelegate *mainDelegate = [[UIApplication sharedApplication] delegate];
    NSDictionary *heroClass = [mainDelegate.heroSkillDataSource objectForKey:@"Monk"];
    NSDictionary *skillType = [heroClass objectForKey:@"Initiative"];
    NSDictionary *skill = [skillType objectForKey:selectedSkillKey];
    NSDictionary *rune = [[skill objectForKey:@"runes"] objectForKey:[runeList objectAtIndex:[indexPath row]]];
    NSString *name = NSLocalizedString([rune objectForKey:@"name"], nil); 
